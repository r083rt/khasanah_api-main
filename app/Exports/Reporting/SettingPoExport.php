...
class SettingPoExport implements FromArray, WithHeadings, WithStyles, WithTitle
{
    protected $id;
    ...
    */
    ...
    public function array(): array
    {
        ...
        foreach ($data->forecastConversionSettingPo as $po) {
            foreach ( $po['forecastConversionSettingPoSuppliers'] as $supplier) {
                foreach( $supplier['forecastConversionSettingPoSupplierDeliveries'] as $delivery) {
                    $results[] = [
                        'supplier' => $supplier['purchasingSupplier']['name'],
        ...
        foreach ($results as $result) {
            ...
            if (!isset($mergedResults[$supplier][$item])) {
                // Initialize a new entry if the supplier-item combination is not already in the merged results
                $mergedResults[$supplier][$item] = $result;
            } else {
                // Merge the quantities for each day
                foreach ($result as $key => $value) {
                    if ($key !== 'supplier' && $key !== 'item') {
                        $mergedResults[$supplier][$item][$key] += $value;
                    }
                }
            }
        }
        ...
        $finalResults = [];
        foreach ($mergedResults as $supplier => $items) {
