<?php

namespace App\Console\Commands\Menu\Reporting;

use App\Models\Menu;
use Illuminate\Console\Command;
use Illuminate\Support\Arr;

class MenuPenjualanPerJenis extends Command
{
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'menu:penjualan-per-jenis';

    /**
     * The console command description.
     *
     * @var string
     */
    protected $description = 'Create Menu Penjualan per jenis';

    /**
     * Create a new command instance.
     *
     * @return void
     */
    public function __construct()
    {
        parent::__construct();
    }

    /**
     * Execute the console command.
     *
     * @return int
     */
    public function handle()
    {
        $data = [
            [
                'title' => "History Pejualan",
                'parent_id' => null,
                'classification' => 'central_report',
                'icon' => 'analytics',
                'url' => '/reporting/sale',
                'type' => 'item',
                'order_menu' => 1,
                'is_displayed' => 1,
                'permissions' => [
                    [
                        'name' => 'Lihat',
                        'action' => 'penjualan.lihat',
                    ]
                ]
            ],
            [
                'title' => "History Barang Masuk",
                'parent_id' => null,
                'classification' => 'central_report',
                'icon' => 'insert_chart',
                'url' => '/reporting/incoming-product',
                'type' => 'item',
                'order_menu' => 2,
                'is_displayed' => 1,
                'permissions' => [
                    [
                        'name' => 'Lihat',
                        'action' => 'item-masuk.lihat',
                    ]
                ]
            ],
            [
                'title' => "Histoy Biaya",
                'parent_id' => null,
                'classification' => 'central_report',
                'icon' => 'multiline_chart',
                'url' => '/reporting/expense',
                'type' => 'item',
                'order_menu' => 3,
                'is_displayed' => 1,
                'permissions' => [
                    [
                        'name' => 'Lihat',
                        'action' => 'biaya.lihat',
                    ]
                ]
            ]
        ];

        foreach ($data as $value) {
            $menu = Menu::where('classification', $value['classification'])->where('title', $value['title'])->first();
            if ($menu) {
                $menu->update(Arr::except($value, ['permissions']));

                foreach ($value['permissions'] as $row) {
                    $permission = $menu->permissions()->update($row);
                }
            } else {
                $menu = Menu::create(Arr::except($value, ['permissions']));
                foreach ($value['permissions'] as $row) {
                    $permission = $menu->permissions()->create($row);
                    $permission->roles()->create(['role_id' => 1]);
                }
            }
        }
    }
}
