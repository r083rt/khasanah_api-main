...
class BrowniesTargetSale extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'production:brownies-target-sale';
     ...
     * @var string
     */
    protected $description = 'Update brownies target sale';
        ...
        try {
            ...
            foreach ($branches as $value) {
                ...
                foreach ($days as $values) {
                    ...
                    $products = BrowniesTargetPlanProduct::select('product_id')->where('day', strtolower($values))->where('branch_id', $value->id)->where('is_production', 1)->pluck('product_id')->unique();
                    $productList = [];
            ...
            foreach ($datas as $value) {
                foreach ($value['datas'] as $row) {
                    foreach ($row['products'] as $values) {
                        BrowniesTargetPlanSale::updateOrCreate(
                            [
