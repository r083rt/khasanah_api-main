...
class PoAdjustmentBrowniesController extends Controller
{
    /**
    ...
    {
        $this->middleware('permission:po-adjustment-bronis.lihat|po-adjustment-bronis.show', [
            'only' => ['index', 'show']
        ]);
        $this->middleware('permission:po-adjustment-bronis.ubah', [
            'only' => ['approval']
        ]);
        $this->model = $model;
    }
    ...
    {
        $data = $this->model->where('status', 'product_incomplete')->brownies()->with(['details', 'createdBy:id,name', 'details.product:id,name,code,product_unit_id', 'details.product.unit:id,name', 'branch:id,name'])->branch()->available()->search($request)->sort($request);
    ...
    public function show($id)
    {
        ...
        foreach ($model->details as $key => $value) {
            $poSjItem = PoSjItem::select('qty_real')->where('po_id', $id)->where('type', 'po_brownies_product')->where('product_id', $value->product_id)->first();
            if (is_null($poSjItem)) {
                $model->details->forget($key);
            } else {
                $value->qty_real = $poSjItem->qty_real;
            }
        }
    ...
    public function approval(Request $request, $id)
    {
        ...
        $data = DB::connection('mysql')->transaction(function () use ($data, $model) {
            ...
            // $poSjItem = PoSjItem::select('branch_id', 'product_id', 'qty', 'qty_real', 'qty_delivery', 'po_sj_id', 'po_date', 'box_name', 'is_added')->whereIn('type', ['po_brownies_product'])->where('po_id', $model->id)->get();
